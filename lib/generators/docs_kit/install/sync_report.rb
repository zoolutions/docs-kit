...
module DocsKit
  module Generators
    ...
    class SyncReport
      ...
      # (see .tailwind_sources_rule); the drift warning consults
      # `git check-ignore` instead, which resolves full pattern semantics.
      ...
      def tailwind_sources_drift
        ...
        # git's own verdict, not the recognized-lines regex: check-ignore
        # resolves the FULL pattern semantics (broad globs, ordering, nested
        # .gitignores), so a dead `!` line before a broader ignore still warns,
        # while a genuinely effective negation (the site's opt-out) stays quiet.
      ...
      # True when the repo's effective exclude rules cover `rel`. `--no-index`
      # is essential: without it a TRACKED path is never reported ignored (the
      # exact state this drift check exists to catch). Only called after
      # tracked_by_git? proved git + a repo exist, so a failure here
      # conservatively reads as "not ignored" → no drift warning.
      def ignored_by_git?(rel)
        system("git", "-C", @root, "check-ignore", "-q", "--no-index", rel,
